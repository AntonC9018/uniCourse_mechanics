...
class RaycastDiagonal(Scene):
    def __init__(self, **kwargs):
        set_default_output("02_raycast_diagonal")
    ...
    def construct(self):
        ...
        # Diagonal directions in grid space:
        #   UP+RIGHT   = (dr=-1, dc=+1) -> visually up-right   -> label ( 1, -1)
        #   UP+LEFT    = (dr=-1, dc=-1) -> visually up-left     -> label (-1, -1)
        #   DOWN+RIGHT = (dr=+1, dc=+1) -> visually down-right  -> label ( 1,  1)
        #   DOWN+LEFT  = (dr=+1, dc=-1) -> visually down-left   -> label (-1,  1)
        #
        # Objects:
        #   (1, 3) = one step UP+RIGHT from player  -> hits at distance 1
        #   (4, 0) = two steps DOWN+LEFT from player -> hits at distance 2
        #   UP+LEFT and DOWN+RIGHT are clear
        object_cells = {(1, 3), (4, 0), (2, 0)}
        ...
        # (dr, dc) in grid space, screen arrow direction, grid-space label (x, y) where y-down
        directions = [
            ((-1, +1), UP + RIGHT,   r"(1,\ -1)"),   # UP+RIGHT:   col+1, row-1
            ((-1, -1), UP + LEFT,    r"(-1,\ -1)"),  # UP+LEFT:    col-1, row-1
            ((+1, -1), DOWN + LEFT,  r"(-1,\ 1)"),   # DOWN+LEFT:  col-1, row+1
            ((+1, +1), DOWN + RIGHT, r"(1,\ 1)"),    # DOWN+RIGHT: col+1, row+1
        ]
        ...
        for (dr, dc), arrow_dir, label_str in directions:
            ...
            arrow_unit = arrow_dir / np.linalg.norm(arrow_dir)
            arrow = Arrow(
                start=player_center,
                end=player_center + arrow_unit * 0.8,
